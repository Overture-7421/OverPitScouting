      ...
      <div className="content-with-filters">
        ...
        <div className="main-content">
          ...
          <div className="stat-grid">
            <StatCard label="Teams Scouted" value={records.length} icon="smart_toy" color="#8b5cf6" />
            <StatCard label="Swerve" value={`${Math.round(swerveCount / records.length * 100)}%`} icon="360" color="#f59e0b" sub={`${swerveCount} robots`} />
            <StatCard label="Path Planner" value={`${Math.round(pathPlannerCount / records.length * 100)}%`} icon="place" color="#22c55e" />
            <StatCard label="CANivore" value={`${Math.round(canivoreCount / records.length * 100)}%`} icon="bolt" color="#ef4444" />
            <StatCard label="Avg Ball Cap" value={frcAvg(records, 'mech_ball_capacity')} icon="sports_soccer" color="#3b82f6" />
            <StatCard label="Perfect Auto" value={`${Math.round(perfectAutoCount / records.length * 100)}%`} icon="gps_fixed" color="#ec4899" sub={`${perfectAutoCount} teams`} />
          </div>
          ...
          <div className="charts-grid">
            <div className="chart-card">
              ...
              <ResponsiveContainer width="100%" height={220}>
                <PieChart>
                  <Pie data={chassisData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} label={({ name, percent }) => `${name} ${Math.round((percent ?? 0) * 100)}%`}>
